...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(UPPER(SUBSTR(Apellido1, 1, 1)));
    DBMS_OUTPUT.PUT_LINE(UPPER(SUBSTR(Apellido2, 1, 1)));
...
BEGIN
    select max(SALARY) into SalarioMaximo from employees where department_id = 100;
...
BEGIN
    select department_name into Nombredepartamento from departments where DEPARTMENT_ID=departmentid;
    select count(*) into empleados from employees where DEPARTMENT_ID=departmentid;
...
DECLARE
    salariomaximo number(10,2);
    salariominimo number(10,2);
    diferencia number(10,2);
    
BEGIN
    select max(SALARY) into salariomaximo from employees;
    select min(SALARY) into salariominimo from employees;
